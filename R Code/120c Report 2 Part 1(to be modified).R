...
## deaths
MI_death = us_death %>%
  dplyr::filter(Province_State == "Michigan",Country_Region == "US")
...
MI_death[1,]
###get the cumulative death toll and confirmed cases
MI_death_sum = apply(MI_death[,12:dim(MI_death)[2]], 2, sum)

MI_confirmed_sum = apply(MI_confirmed[,12:dim(MI_confirmed)[2]], 2, sum)

# FL data creation -----------------------------------------------------

FL_death = us_death %>%
  dplyr::filter(Province_State == "Florida",Country_Region == "US")
...
date_selected=seq.Date(start_date, end_date, by=1)
par(mfrow=c(1,2))
plot(date_selected,MI_confirmed_selected,xlab='date',ylab='cumulative observed confirmed cases',type='l')
plot(date_selected,MI_death_selected,xlab='date',ylab='cumulative death toll',type='l')
dev.off() ##close it
...
daily_confirmed_MI_df %>%
  ggplot(aes(x=date, y=value)) +
  geom_bar(stat = 'identity', color="white",  fill="#ff8540", width = 1) +
  ylab("Daily confirmed cases in MI")+
  xlab("Date")+ 
  theme(text = element_text(size = 20),
        legend.title = element_text(size = 15),
        legend.text = element_text(size = 15),
...
###plot  the smoothed positive rates 
plot(date_selected, MI_test_PositiveRate_smoothed_selected,type='l',xlab='date',ylab='-day averaged daily positive rate')
...
daily_confirmed_FL_df %>%
  ggplot(aes(x=date, y=value)) +
  geom_bar(stat = 'identity', color="white",  fill="#ff8540", width = 1) +
  ylab("Daily confirmed cases in FL")+
  xlab("Date")+ 
  theme(text = element_text(size = 20),
        legend.title = element_text(size = 15),
        legend.text = element_text(size = 15),
...
# 3.a.2 7-day test+ FL-------------------------------------------------------------------
  ...
  ###plot  the smoothed positive rates 
  plot(date_selected, FL_test_PositiveRate_smoothed_selected,type='l',xlab='date',ylab='-day averaged daily positive rate')
...
##Need the raw number too, get last value of county_death_selected

# 4.c.2 SB cumulative death Oct 31-------------------------------------------------
...
paste("There were ", county_death_selected, "deaths on ", end_date)
...
# 7-day state county cases per capita -----------------------------------------------
...
MI_daily_confirm_rate_selected_joint %>%
  ggplot(aes(long, lat, group = group, fill = Ratio*100)) +
  scale_fill_gradient(low = "yellow", high = "red", na.value = "grey90")+
  geom_polygon(col = "black") +
  coord_map(projection = "albers", lat0 = 10, lat1 = 45) +
...
# 2.c.1 MI cty map Oct 22 ------------------------------------------------------------

# set the date for map the end date you selected before
date_for_map = as.Date("2020-10-22")
...
# 2.c.2 MI cty map Oct 24 -------------------------------------------------------------------


# set the date for map the end date you selected before
date_for_map = as.Date("2020-10-24")
...
# 2.c.3  MI cty map Oct 26 -------------------------------------------------------------------

# set the date for map the end date you selected before
date_for_map = as.Date("2020-10-26")
...
# 3.b.1 FL confirmed cty map Jul 20 -------------------------------------------------------------------
...
# select the daily confirm cases for CA counties
FL_daily_confirm = us_daily_confirm_clean%>%
  filter(Province_State == "Florida")

# This shape file contains the coordinates for county boundaries
FL_shape_data = counties %>%
  filter(state_name == "Florida")

# get the population for CA counties
FL_population = us_daily_death_clean %>%
  filter(Province_State == "Florida")%>%
  dplyr::select(Population)
FL_population = as.numeric(FL_population$Population)

# extract the daily confirmed cases on the selected date date_for_map
FL_daily_confirm_selected = FL_daily_confirm[, c(1:11, 11+ which(all_dates == date_for_map))]

# the Ratio = daily confirmed cases / population
FL_daily_confirm_rate_selected = FL_daily_confirm_selected
FL_daily_confirm_rate_selected[,12] = FL_daily_confirm_selected[,12]/FL_population
FL_daily_confirm_rate_selected[,12][FL_daily_confirm_rate_selected[,12]==0]=NA
colnames(FL_daily_confirm_rate_selected)[12] = "Ratio"

##This is  the county and rate
FL_daily_confirm_rate_selected[,c(11,12)]

# joint the daily confirmed cases with the shape file
FL_daily_confirm_rate_selected_joint <- left_join(FL_daily_confirm_rate_selected, FL_shape_data, by = "county_fips")

# find the lower and upper liFLts of the ratio
range(FL_daily_confirm_rate_selected$Ratio*100, na.rm = T)

FL_daily_confirm_rate_selected_joint %>%
  ggplot(aes(long, lat, group = group, fill = Ratio*100)) +
  scale_fill_gradient(low = "yellow", high = "red", na.value = "grey90")+
  geom_polygon(col = "black") +
  coord_map(projection = "albers", lat0 = 10, lat1 = 45) +
...
# 1b 7-day case/capita county map -----------------------------------
...
date_for_map = as.Date("2020-4-20")
...
date_for_map = as.Date("2020-6-20")
...
date_for_map = as.Date("2020-9-20")
